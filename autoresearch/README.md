...
**3. On first use in a repo, Codex forces a full plan-mode initialization. After that, it scans, confirms, and iterates autonomously:**
...
Each improvement stacks. Each failure reverts. Everything is logged in both runtime artifacts and the unified project document system.
...
A Codex skill that runs a modify-verify-decide loop on your codebase while also maintaining a built-in project operating system. Each iteration makes one atomic change, verifies it against a mechanical metric, and keeps or discards the result. Progress accumulates in git; failures auto-revert. Project state stays recoverable from documents.
...
Stateful artifact updates are backed by bundled helper scripts under `<skill-root>/scripts/`, but most users should keep using the single human-facing entrypoint: **`$codex-autoresearch`**. Here `<skill-root>` means the directory containing the loaded execution `SKILL.md`; in the common repo-local install this is `.agents/skills/codex-autoresearch/autoresearch`.
...
- `execution_policy` applies only to paths that spawn nested Codex sessions: background managed runs and `exec`. In this skill the default is sandboxed `workspace_write`, and callers should opt into `danger_full_access` only when they explicitly need unrestricted nested sessions.
...
**How do I use it in CI?** Use `Mode: exec` or `codex exec`. In controlled automation environments, this skill defaults to sandboxed `workspace_write`, so prefer `codex exec --full-auto ...` unless you explicitly need unrestricted `danger_full_access`. Configure Codex CLI authentication first; API key auth is preferred for CI/programmatic use. All config is provided upfront, output is JSON, and exit codes indicate success/failure.
