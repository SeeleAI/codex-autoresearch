...
10. Each background runtime cycle launches a non-interactive `codex exec` session with the runtime prompt supplied on stdin.
   Background launch manifests carry an `execution_policy`; this skill now defaults to sandboxed `workspace_write`, and only uses `danger_full_access` when you explicitly ask for unrestricted execution.
