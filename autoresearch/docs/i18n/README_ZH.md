...
Codex: 在 src/**/*.ts 中找到 47 个 `any`。
       ...
       先选运行模式，再回复 "go" 开始，或告诉我要改什么。

你:    Background，go。跑一晚上。

Codex: 开始后台运行 -- 基线：47。分离的 runtime 已经在持续迭代。
```
...
下图先展示交互式启动流程，再展示共享的循环核心。进入循环之前，Codex 会先探测环境、检查是否存在可恢复会话、确认配置，并要求显式选择 `foreground` 或 `background`。

```
              +----------------------+
              |      环境探测        |  <-- 检测 CPU/GPU/RAM/工具链
              +----------+-----------+
                         |
              +----------v-----------+
              |     会话恢复?        |  <-- 检查既有 results/state
              +----------+-----------+
                         |
              +----------v-----------+
              |     读取上下文       |  <-- scope + 经验 + repo 状态
              +----------+-----------+
                         |
              +----------v-----------+
              |    wizard 确认       |  <-- goal/metric/verify/guard
              | + 选择运行模式       |      + foreground 或 background
              +----------+-----------+
                         |
               +---------+---------+
               |                   |
     +---------v--------+  +-------v---------+
     | 前台运行         |  | 后台运行        |
     | 当前会话内       |  | launch manifest |
     | 无 runtime 文件  |  | + detached ctl  |
     +---------+--------+  +-------+---------+
               |                   |
               +---------+---------+
                         |
              ...
              +----------+-----------+
                         |
              +----------v-----------+
              |   supervisor 结果    |  <-- continue / stop / needs_human
              +----------------------+
```

Foreground 和 background 共享完全相同的实验协议。差别只在于循环运行的位置：foreground 在当前 Codex 会话内执行，background 则交给 detached runtime controller。无界运行会持续到你中断，或到达其他终止条件（目标/停止条件满足、soft blocker handoff、或 hard blocker）；有界运行遵循同样的终止条件，但也会在 `Iterations: N` 时停止。
...
PHASE 1: 读取上下文 + 经验文件
PHASE 2: 确认配置 + 选择 foreground 或 background

IF foreground:
  在当前 Codex 会话中运行循环
ELSE background:
  写入 autoresearch-launch.json 并启动 detached runtime
...
共享循环 (永远 or N 次):
  ...
  10. 重复，直到满足停止条件、用户手动停止、进入 needs_human，或达到配置的迭代上限。
```
...
最少一轮确认，最多五轮。然后你先选 `foreground` 或 `background`，再说 "go"。`foreground` 会在当前会话里继续迭代，`background` 会交给 detached runtime，让你可以离开。
如果你想真正无人值守地跑起来，需要提前把 Codex CLI 的审批 / 沙箱权限配置到不会打断 `git commit` 或 `git revert`。在一次性或可信仓库里，直接给 Codex 更高权限是最简单的做法。
如果你的目标除了数值阈值外还有结构性要求，Codex 也可以用结构化标签来控制停止。例如：“只有当延迟 <= 120 ms，且保留结果带有 `production-path` 和 `real-backend` 标签时才停止。” 这样可以避免因为错误机制、子系统或实现路径带来的数值改进而误停。
...
Codex: 开始后台运行。先建立基线，然后进入 detached iterations。
```
...
- 每次 PIVOT 决策后提取策略经验
- 运行结束时提取总结经验
- 容量目标：历史归档大致保持在 50 条左右，同时完整保留当前 run 的 lessons；更早的历史会按时间衰减汇总
...
| 3 次 PIVOT 无改善 | **软阻塞** -- 停止当前 run，并报告需要人工输入、更宽的 scope 或更好的指标 |
...
如果 Codex 在交互模式下检测到之前被中断的运行，它可以从最后一致的状态恢复，而不是从头开始。主要恢复来源是 `autoresearch-state.json`，一个每次迭代原子更新的紧凑状态快照。`exec` 模式下，状态只会写入 `/tmp/codex-autoresearch-exec/...` 下的临时文件，并需要由 `exec` 工作流在退出前显式清理。foreground 恢复只依赖 `research-results.tsv` 和 `autoresearch-state.json`；要让分离的 background 运行控制器直接恢复，仍然必须已经存在 `autoresearch-launch.json`。

恢复优先级（交互模式）：

1. **JSON + TSV 摘要一致：** 立即恢复；background 运行还需要已确认的 launch manifest
2. **JSON 有效，但 helper 报告不一致：** 迷你向导（1 轮确认）
3. **JSON 缺失或损坏，TSV 存在：** helper 先重建保留状态供确认，然后按所选模式继续
...
- **`autoresearch-state.json`** -- 用于 foreground 快速恢复和共享保留状态恢复的紧凑状态快照
- **`autoresearch-launch.json`** -- 仅 background 运行使用的已确认 launch manifest
- **`autoresearch-runtime.json`** -- background runtime 控制状态（PID、status、last decision）
- **`autoresearch-runtime.log`** -- 长时间 background 运行的 runtime 日志
...
`exec` 模式下，状态快照仅保存在 `/tmp/codex-autoresearch-exec/...` 的临时位置，并通常由 `autoresearch_exec_state.py --cleanup` 在退出前删除。只应在最后一个有状态 helper 调用完成后再做清理。默认 helper 流程还会在新的 exec run 开始前，自动将 repo 根目录中的 `research-results.tsv` 和 `autoresearch-state.json` 归档为 `research-results.prev.tsv` 和 `autoresearch-state.prev.json`。
...
如果你不是在做 control plane 本身的自动化或调试，就到这里为止，忽略下面这些原始 helper 命令。
...
| 上下文漂移（长时间运行） | 每 10 次迭代执行协议指纹检查；发生压缩后提高检查频率；失败时从磁盘重新读取 |
...
**怎么停止？** `foreground` 模式下直接中断当前 Codex 会话。`background` 模式下，可以让 `$codex-autoresearch` 停掉当前运行；如果你在直接自动化后端，也可以调用 `python3 <skill-root>/scripts/autoresearch_runtime_ctl.py stop --repo <repo>`。`Iterations: N` 和停止条件也一样生效。
...
**中断后能恢复吗？** 可以。foreground 运行从 `research-results.tsv` 和 `autoresearch-state.json` 恢复；background 运行还需要已有的 `autoresearch-launch.json`。缺少已确认 launch state 时，应按正常 background 启动流程重新开始。
