...
Codex: src/**/*.ts에서 47개의 `any`를 발견했습니다.
       ...
       추가 확인 필요:
       - 실행 모드: foreground 와 background 중 무엇으로 할까요?
       - 전부 제거할 때까지 실행할까요, 아니면 N회 반복으로 제한할까요?

       실행 모드를 고른 뒤 "go"라고 답하면 시작합니다.
       변경할 사항이 있으면 알려주세요.

사용자: Background, go. 밤새 돌려줘.

Codex: background 실행을 시작합니다 -- 베이스라인: 47.
       detached runtime 이 이미 반복을 이어가고 있습니다.
```
...
아래 다이어그램은 먼저 대화형 시작 흐름을, 그다음 공유되는 루프 코어를 보여줍니다. 루프에 들어가기 전에 Codex는 환경을 점검하고, 재개 가능한 세션이 있는지 확인하고, 설정을 확정한 뒤, `foreground` 와 `background` 중 하나를 반드시 선택하게 합니다.

```
              +----------------------+
              |   환경 탐지          |  <-- CPU/GPU/RAM/툴체인 감지
              +----------+-----------+
                         |
              +----------v-----------+
              | 세션 재개?           |  <-- 이전 결과/상태 확인
              +----------+-----------+
                         |
              +----------v-----------+
              | 컨텍스트 읽기        |  <-- scope + 교훈 + repo 상태
              +----------+-----------+
                         |
              +----------v-----------+
              | wizard 확인          |  <-- 목표/지표/verify/guard
              | + 모드 선택          |      + foreground 또는 background
              +----------+-----------+
                         |
               +---------+---------+
               |                   |
     +---------v--------+  +-------v---------+
     | foreground 실행  |  | background 실행 |
     | 현재 세션        |  | manifest + ctl  |
     | runtime 파일 없음|  | detached runtime|
     +---------+--------+  +-------+---------+
               |                   |
               +---------+---------+
                         |
              ...
              +----------+-----------+
                         |
              +----------v-----------+
              | supervisor 결과      |  <-- continue / stop / needs_human
              +----------------------+
```

Foreground 와 background 는 동일한 실험 프로토콜을 공유합니다. 차이는 루프가 어디에서 실행되느냐뿐입니다. foreground 는 현재 Codex 세션 안에서, background 는 detached runtime controller 에서 실행됩니다. 무제한 실행은 당신이 중단하거나 다른 종료 조건(목표/중지 조건 달성, soft blocker handoff, 또는 hard blocker)에 도달할 때까지 계속됩니다. 제한 실행은 같은 종료 조건을 따르되 `Iterations: N` 에서도 멈춥니다.

**의사 코드:**

```
PHASE 0: 환경을 탐색하고, 재개 가능한 세션이 있는지 확인
PHASE 1: 컨텍스트 + 교훈 파일 읽기
PHASE 2: 설정을 확인하고 foreground 또는 background 선택

IF foreground:
  현재 Codex 세션에서 루프 실행
ELSE background:
  autoresearch-launch.json 을 기록하고 detached runtime 시작
...
공유 루프 (영구 or N회):
  ...
  3. 원자적 변경 하나 수행
  4. 기계적 verify + guard 후 governed commit
  5. 기계적 검증 + guard 실행
  6. 개선 -> 유지 (교훈 추출). 악화 -> 승인된 롤백 전략. 크래시 -> 수정 또는 스킵.
  7. 결과 기록
  8. 상태 점검 (디스크, git, 검증 건전성)
  9. 3회 이상 연속 폐기 -> REFINE, 5회 이상 -> PIVOT, 2회 PIVOT -> Web 검색
  10. 중지 조건, 수동 중지, `needs_human`, 또는 설정된 반복 상한에 도달할 때까지 반복한다.
```
...
최소 1라운드, 최대 5라운드의 확인이 있습니다. 그 다음 `foreground` 또는 `background` 를 고르고 "go"라고 말합니다. `foreground` 에서는 현재 세션에서 계속 반복하고, `background` 에서는 detached runtime 으로 넘긴 뒤 자리를 비울 수 있습니다.
...
목표에 수치 임계값 외에 구조적 성공 조건도 있다면, Codex는 구조화된 라벨로 종료를 제어할 수도 있습니다. 예를 들어 "지연 시간이 120 ms 이하이고, 유지된 keep 에 `production-path` 와 `real-backend` 라벨이 붙어 있을 때만 멈춘다"처럼 설정할 수 있습니다. 이렇게 하면 잘못된 메커니즘, 서브시스템, 구현 경로에서 나온 수치 개선으로 성급하게 멈추는 일을 막을 수 있습니다.
...
Codex: background 실행을 시작합니다. 먼저 베이스라인을 만들고, 그다음 detached iteration 으로 넘어갑니다.
```
...
사용자: Foreground, go. 전부 수정하고 ruff 가드도 사용해.
```
...
Codex: 감지: main으로의 PR, 3개 커밋.
       ...
       실행 모드를 먼저 고른 뒤 선호 사항을 알려주세요.

사용자: Foreground, 먼저 드라이런.
```
...
- 실행 완료 시 요약 교훈 기록
- 용량 목표: 현재 run 의 lessons 는 그대로 보존하면서 전체 히스토리 아카이브를 대략 50개 수준으로 유지하고, 더 오래된 이력은 시간 감쇠와 함께 요약
...
| 개선 없는 PIVOT 3회 | **소프트 블로커** -- 현재 run 을 중단하고, 사람의 판단이나 더 넓은 scope 또는 더 나은 metric 이 필요하다고 보고 |
...
Codex가 인터랙티브 모드에서 이전에 중단된 run 을 감지하면 처음부터 다시 시작하는 대신 마지막 일관된 상태에서 재개할 수 있습니다. 주요 복구 소스는 `autoresearch-state.json`으로, 각 반복마다 원자적으로 업데이트되는 컴팩트한 상태 스냅샷입니다. `exec` 모드에서는 상태가 `/tmp/codex-autoresearch-exec/...` 아래의 임시 파일에만 존재하며, `exec` 워크플로가 종료 전에 이를 명시적으로 정리해야 합니다. `foreground` 는 `research-results.tsv` 와 `autoresearch-state.json` 으로 직접 재개하고, 분리된 `background` 런타임 컨트롤러가 직접 재개하려면 기존 `autoresearch-launch.json` 이 계속 필요합니다.

복구 우선순위(인터랙티브 모드):

1. **JSON + TSV 요약 일치:** 즉시 재개하며, `background` 실행은 여기에 더해 확인된 launch manifest 가 필요합니다
2. **JSON 은 유효하지만 helper 가 불일치를 보고:** 미니 마법사 (1라운드 확인)
3. **JSON 없음 또는 손상, TSV 존재:** 보조 스크립트가 유지 상태를 재구성해 확인한 뒤 선택한 모드로 계속 진행
...
CI에서 `codex exec`를 사용하기 전에 Codex CLI 인증을 미리 구성하세요. 통제된 자동화 환경에서는 독립적인 `exec` 실행도 managed runtime 의 기본 샌드박스 정책 `workspace_write` 와 맞추기 위해 `codex exec --full-auto ...` 사용을 권장합니다. 제한 없는 중첩 세션이 정말 필요한 경우에만 `danger_full_access` 를 명시적으로 사용하세요. 프로그래밍 방식 실행에는 API key 인증이 권장됩니다.
...
- **`autoresearch-state.json`** -- `foreground` 빠른 재개와 공유 retained state 복구를 위한 컴팩트한 상태 스냅샷
- **`autoresearch-launch.json`** -- `background` 실행 전용의 확인된 launch manifest
- **`autoresearch-runtime.json`** -- `background` runtime 제어 상태(PID, status, last decision)
- **`autoresearch-runtime.log`** -- 장시간 `background` 실행용 runtime 로그
...
`exec` 모드에서는 상태 스냅샷이 `/tmp/codex-autoresearch-exec/...` 임시 위치에만 존재하며, 보통 종료 전에 `autoresearch_exec_state.py --cleanup` 으로 정리됩니다. 이 cleanup 은 마지막 stateful helper 호출이 끝난 뒤에만 실행해야 합니다. 기본 helper 흐름은 새 `exec` run 을 시작하기 전에 repo 루트의 `research-results.tsv` 와 `autoresearch-state.json` 을 `research-results.prev.tsv` 와 `autoresearch-state.prev.json` 으로 자동 보관하기도 합니다.
...
control-plane 자체를 자동화하거나 디버깅하는 것이 아니라면 여기서 멈추고 아래의 raw helper 명령은 무시해도 됩니다.
...
- 첫 대화형 실행에서는 목표를 자연스럽게 설명하고, 확인 질문에 답하고, `foreground` 또는 `background` 를 명시적으로 고른 뒤 `go`라고 말하면 됩니다
...
- `execution_policy` 는 중첩 Codex 세션을 시작하는 경로, 즉 `background` 와 `exec` 에만 적용됩니다. 이 skill 의 기본값은 샌드박스된 `workspace_write` 입니다
...
| 컨텍스트 드리프트 (장시간 실행) | 10회 반복마다 프로토콜 핑거프린트 검사; compaction 이후에는 빈도를 높이고, 실패 시 디스크에서 재로드 |
...
codex-autoresearch/
  ...
  references/
    ...
    runtime-hard-invariants.md      # 간결한 런타임 체크리스트
    loop-workflow.md                # 얇은 loop 실행 가이드
    autonomous-loop-protocol.md     # 상세 루프 참고 문서
...
**어떻게 멈추나요?** `foreground` 에서는 현재 활성 Codex 세션을 중단하세요. `background` 에서는 `$codex-autoresearch` 에 현재 실행을 멈추라고 요청하거나, 백엔드를 직접 자동화한다면 `python3 <skill-root>/scripts/autoresearch_runtime_ctl.py stop --repo <repo>` 를 호출하세요. `Iterations: N` 과 중지 조건도 계속 사용할 수 있습니다.
...
**중단 후 재개할 수 있나요?** 예. `foreground` 는 `research-results.tsv` 와 `autoresearch-state.json` 에서 재개하고, `background` 는 여기에 더해 `autoresearch-launch.json` 도 필요합니다. 확인된 launch state 가 없으면 일반 `background` launch 흐름으로 새 run 을 시작하세요.

**Web 검색이 가능한가요?** 예. 여러 번의 전략 피봇 후 정체되었을 때 사용됩니다. Web 검색 결과는 가설로 취급되어 기계적으로 검증됩니다.

**CI에서 사용하려면?** `Mode: exec` 또는 `codex exec`를 사용하세요. 통제된 자동화 환경에서는 기본 runtime 의 샌드박스된 `workspace_write` 권한과 맞추기 위해 `codex exec --full-auto ...` 사용을 권장합니다. 제한 없는 중첩 세션이 명시적으로 필요할 때만 `danger_full_access` 로 전환하세요. 모든 설정은 사전에 제공되며, 출력은 JSON 형식이고, 종료 코드가 성공/실패를 나타냅니다.
