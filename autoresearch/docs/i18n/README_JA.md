...
Foreground と background は、同じ実験プロトコルを共有します。違うのはループがどこで実行されるかだけです。foreground は現在の Codex セッション内で、background は detached runtime controller で実行されます。無制限実行は、あなたが中断するか、別の終端条件（目標/停止条件の達成、soft blocker handoff、または hard blocker）に達するまで続きます。上限制実行は同じ終端条件に従いますが、`Iterations: N` でも停止します。
...
- 実行完了時にサマリー教訓を記録
- 容量目標：履歴アーカイブをおおむね 50 エントリ前後に保ちつつ、現在の run の lessons はそのまま保持し、古い履歴は時間減衰つきで要約
...
| 改善なしの PIVOT 3 回 | **ソフトブロッカー** -- 現在の run を停止し、人の判断、より広い scope、またはより良い metric が必要だと報告 |
...
1. **JSON + TSV の要約が一致：** 即座に再開。`background` 実行では追加で確認済みの launch manifest が必要
2. **JSON は有効だが helper が不一致を報告：** ミニウィザード（1 ラウンド確認）
3. **JSON 欠落または破損、TSV 存在：** 補助スクリプトが保持状態を再構築して確認し、その後は選択したモードで続行
...
- **`autoresearch-state.json`** -- `foreground` の高速再開と共有 retained state の復元に使うコンパクトな状態スナップショット
- **`autoresearch-launch.json`** -- `background` 実行専用の確認済み launch manifest
- **`autoresearch-runtime.json`** -- `background` runtime の制御状態（PID、status、last decision）
- **`autoresearch-runtime.log`** -- 長時間の `background` 実行向け runtime ログ
...
`exec` モードでは状態スナップショットは `/tmp/codex-autoresearch-exec/...` の一時領域にのみ存在し、通常は終了前に `autoresearch_exec_state.py --cleanup` で削除されます。この cleanup は最後の stateful helper 呼び出しが終わってから行ってください。既定の helper フローは、新しい `exec` run を始める前に repo 直下の `research-results.tsv` と `autoresearch-state.json` も `research-results.prev.tsv` と `autoresearch-state.prev.json` に自動退避します。
...
control-plane 自体を自動化またはデバッグしているのでなければ、ここで止めて以下の生の helper コマンドは無視して構いません。
...
**どうやって止める？** `foreground` ではアクティブな Codex セッションを中断します。`background` では `$codex-autoresearch` に現在の実行を止めるよう頼むか、バックエンドを直接自動化しているなら `python3 <skill-root>/scripts/autoresearch_runtime_ctl.py stop --repo <repo>` を呼び出してください。`Iterations: N` と停止条件もそのまま使えます。
