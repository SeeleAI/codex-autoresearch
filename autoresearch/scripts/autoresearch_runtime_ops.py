...
WINDOWS_LAUNCH_SUFFIXES = (".cmd", ".bat", ".exe")


def resolve_codex_bin_path(codex_bin: str) -> str:
    if os.name != "nt":
        return codex_bin

    candidate = Path(codex_bin)
    if not (candidate.is_absolute() or "/" in codex_bin or "\\" in codex_bin):
        return codex_bin
    if candidate.suffix.lower() in WINDOWS_LAUNCH_SUFFIXES and candidate.exists():
        return str(candidate)

    for suffix in WINDOWS_LAUNCH_SUFFIXES:
        suffixed = Path(f"{codex_bin}{suffix}")
        if suffixed.exists():
            return str(suffixed)
    if candidate.exists():
        return str(candidate)
...
def runtime_process_group_id(pid: int) -> int | None:
    if os.name == "nt":
        return pid
    return os.getpgid(pid)


def current_runtime_process_group_id() -> int | None:
    if os.name == "nt":
        return os.getpid()
    return os.getpgid(0)


def runtime_session_popen_kwargs() -> dict[str, Any]:
    if os.name == "nt":
        return {"creationflags": getattr(subprocess, "CREATE_NEW_PROCESS_GROUP", 0)}
    return {"start_new_session": True}


def signal_runtime_process(pid: int, pgid: int | None, *, force: bool) -> None:
    if os.name == "nt":
        ...
        if completed.returncode != 0 and pid_is_alive(pid):
            detail = (completed.stderr or completed.stdout or "").strip()
            raise OSError(detail or f"taskkill exited with status {completed.returncode}")
        return

    target_group = int(pgid or pid)
    os.killpg(target_group, signal.SIGKILL if force else signal.SIGTERM)


def maybe_attach_progress_snapshot(
    ...
    *,
    ...
    if not project_system_status(repo)["initialized"]:
        enriched["progress_snapshot"] = None
        enriched["progress_snapshot_status"] = "uninitialized_project_system"
        return enriched
    try:
        snapshot_payload = (
            ...
            if persist
            else calculate_progress_snapshot(
                results_path=results_path,
                state_path_arg=state_path_arg,
            )
        )
    except (AutoresearchError, OSError, ValueError, TypeError) as exc:
        enriched["progress_snapshot"] = None
        enriched["progress_snapshot_status"] = "error"
        enriched["progress_snapshot_error"] = str(exc)
        return enriched
    enriched["progress_snapshot"] = snapshot_payload.get("current_snapshot")
    enriched["progress_snapshot_status"] = "persisted" if persist else "calculated"
    return enriched
...
def runtime_summary(
    *,
    ...
    if runtime_error is not None:
        return maybe_attach_progress_snapshot({
            "status": "needs_human",
            "runtime_path": str(runtime_path),
            "log_path": "",
        ...
        }, repo=repo, results_path=results_path, state_path_arg=str(resolved_state_path))
    ...
    if runtime is not None and runtime.get("status") == "needs_human":
        return maybe_attach_progress_snapshot(persisted_runtime_summary(
            runtime=runtime,
...
def start_runtime(args: argparse.Namespace, *, runner_path: Path) -> dict[str, Any]:
    ...
    }, repo=repo, results_path=results_path, state_path_arg=state_path_arg)
...
def stop_runtime(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if pid_is_alive(pid):
        kill_label = "force kill" if os.name == "nt" else "SIGKILL"
        try:
            signal_runtime_process(int(pid), int(pgid) if pgid is not None else None, force=False)
        except (OSError, ProcessLookupError):
            pass
        stopped_after_term = wait_for_process_exit(pid, timeout=args.grace_seconds)
        if not stopped_after_term:
            try:
                signal_runtime_process(int(pid), int(pgid) if pgid is not None else None, force=True)
            except (OSError, ProcessLookupError):
                pass
            stopped_after_kill = wait_for_process_exit(pid, timeout=STOP_KILL_WAIT_SECONDS)
            if not stopped_after_kill:
                error = f"Runtime process {pid} remained alive after {kill_label}."
                runtime["status"] = "needs_human"
                ...
                return maybe_attach_progress_snapshot({
                    ...
                    "reason": "stop_failed",
                    "error": error,
                }, repo=repo, results_path=Path(str(runtime["results_path"])), state_path_arg=str(runtime["state_path"]))
