...
def main() -> int:
    ...
    if args.command == "status":
        if args.repo:
            ...
            if state_path is not None and state_path.is_absolute():
                repo = state_path.parent
            else:
                requested_results_path = Path(args.results_path)
                repo = (
                    results_repo_root(requested_results_path)
                    if requested_results_path.is_absolute()
                    else resolve_repo_path(None)
                )
