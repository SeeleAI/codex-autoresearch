...
def replace_file_atomic(tmp_name: str, path: Path, *, attempts: int = 5, delay_seconds: float = 0.05) -> None:
    ...
    if last_error is not None:
        try:
            with open(tmp_name, "rb") as src, open(path, "wb") as dst:
                dst.write(src.read())
                dst.flush()
                os.fsync(dst.fileno())
...
def cleanup_temp_file(tmp_name: str, *, attempts: int = 5, delay_seconds: float = 0.05) -> None:
    for attempt in range(attempts):
        try:
            os.unlink(tmp_name)
            return
...
def write_json_atomic(path: Path, payload: dict[str, Any]) -> None:
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            ...
            f.flush()
            os.fsync(f.fileno())
        replace_file_atomic(tmp_name, path)
