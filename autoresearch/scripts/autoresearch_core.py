...
def command_is_executable(command: str) -> bool:
    ...
    if candidate.is_absolute() or "/" in executable or "\\" in executable:
        ...
        if os.name == "nt" and candidate.suffix.lower() not in {".cmd", ".bat", ".exe"}:
            candidates = [Path(f"{candidate}{suffix}") for suffix in (".cmd", ".bat", ".exe")] + candidates
        return any(item.is_file() and os.access(item, os.X_OK) for item in candidates)
