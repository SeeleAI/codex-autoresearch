...
Here `<skill-root>` is the directory containing the loaded execution `SKILL.md`. In the common repo-local install this is usually `.agents/skills/codex-autoresearch/autoresearch`, so the exact command becomes `python3 .agents/skills/codex-autoresearch/autoresearch/scripts/...`.
...
1. Route git adoption, dirty-worktree ownership, branch strategy, artifact retention, and `.gitignore` generation through `../git-runtime-governor/SKILL.md` before launch whenever the run depends on iterative commits.
2. Confirm each managed repo is either already under git or has an explicit adoption decision recorded by `git-runtime-governor`.
3. Inspect `git status --porcelain` for each managed repo under that stored policy.
4. If unrelated user changes are present, do not start the commit/revert loop until the wizard resolves them through the git governor.
5. Confirm the scope resolves to real files.
6. Confirm the verify command exists and is plausible for this repo.
7. If a guard exists, confirm it is a pass/fail command.
8. If destructive rollback may be needed, get approval during setup before launch and prefer a dedicated experiment branch/worktree for unattended runs.
...
Their git treatment is governed by `git-runtime-governor`, not by a blanket exclusion rule. Unresolved categories default to conservative ignore. A category becomes stageable only when the persisted policy in `.agent-os/autoresearch-config.md` explicitly allows it.
...
- Ask `git-runtime-governor` to classify the changed files under the stored or newly captured policy.
- In interactive modes, route the ownership question through the staged questionnaire: are these changes part of the managed experiment, pre-existing user work that must stay untouched, or files that should be ignored by policy?
- If the user confirms the changes are part of the experiment and they are in-policy, continue.
- If the user says no, suggest `plan` mode or a clean branch/worktree.
- In `exec` mode, unresolved pre-existing changes remain a hard blocker. Do not ask; emit the blocker and exit with code 2.
...
## Phase 5: Workspace Isolation Check

Before verification, confirm the hypothesis still fits one focused change and the worktree stays within the managed repo set and scope.

Rules:

- read the current diff before verify so you know what the hypothesis actually changed,
- if there is no diff, log `no-op` and move on (counts toward the consecutive-discard threshold for stuck recovery),
- do not stage or commit yet; the actual git action belongs to `git-runtime-governor` after verification.

If the workspace is not safe for iterative commits, log a hard blocker and stop the loop. Do not ask -- report the situation in the completion summary.
...
## Phase 6.7: Git Governance Auto-Commit

After verification and any guard handling, but before the round is finalized, route to `../git-runtime-governor/SKILL.md`.

Required sequence:

```bash
python3 ../git-runtime-governor/scripts/git_runtime_governor.py commit-message ...
git add -- <in-policy-files>
git diff --cached --name-only
git commit -m "<governed message>"
```

Rules:

- read the persisted git policy from `.agent-os/autoresearch-config.md`,
- refresh the managed `.gitignore` block first if policy changed in this session,
- stage all in-policy modifications for each managed repo, including categories the user explicitly allowed,
- never stage unresolved or out-of-policy files; unresolved defaults to ignore,
- inspect the staged file list before committing,
- if there is no in-policy diff, treat the round as `no-op` and continue to decision/logging without inventing a commit,
- use a structured message that includes iteration, mode, summary, and policy fingerprint,
- treat lower-level helpers such as `autoresearch_commit_gate.py` as enforcement components under this governance layer, not as separate policy authorities.

If the workspace is not safe for the governed commit, log a hard blocker and stop the loop. Do not ask -- report the situation in the completion summary.
...
Rollback follows the strategy approved during setup. If Phase 6.7 created the iteration commit and the round is discarded, roll back that governed commit using the approved mechanism:
...
The results log is governed by the stored git policy. If the user allows autoresearch state files to be committed, they may be included; otherwise they remain ignored by policy.
...
- **Commit safety at the same boundary:** when the managed repos are git-backed, `autoresearch_runtime_ctl.py` also runs `autoresearch_commit_gate.py` with the launch-manifest repo list and per-repo scopes before each detached session. This remains a lower-level enforcement gate under `git-runtime-governor`; relaunch is blocked if staged out-of-policy or out-of-scope changes are present in any managed repo.
