...
For the first initialization or adoption of an uninitialized repo, this wizard becomes a forced plan-mode interrogation pass. Keep asking until there are no unresolved human-owned decisions left.

Before that initialization/adoption handoff begins under the root wrapper, run an `env-bootstrap` environment probe first so hardware visibility and tool completeness are known.

Before initialization/adoption or first recovery questioning is considered complete, route the git-specific decisions through `../git-runtime-governor/SKILL.md` so repo adoption, dirty-worktree ownership, branch strategy, artifact retention, and `.gitignore` policy are explicit.
...
7. For the first forced initialization or adoption of an uninitialized repo, ignore the normal soft cap and keep drilling until there are no high-impact human judgment gaps.
8. Present a structured confirmation summary before launching (see Confirmation Format below).
9. The mandatory confirmation round must never collapse into a bare "foreground/background + go" prompt. Even if the only unresolved choice is run mode, first show a short repo-grounded summary of the confirmed goal, metric, verify path, and next step.
...
- For the first forced initialization/adoption pass, ignore the normal 5-round cap and keep asking in grouped batches until there are no unresolved human-owned decisions.
...
- Background execution policy: `workspace_write` by default; upgrade to `danger_full_access` only if you explicitly need unrestricted nested sessions
...
**Need to confirm**
- Run mode: foreground, or background with `workspace_write` / `danger_full_access`?
...
- 后台执行权限：默认 `workspace_write`；只有在明确需要无沙箱嵌套会话时才升级到 `danger_full_access`
- 必需保留标签：`production-path`（仅在保留结果必须来自特定机制/路径时展示）
- 必需停止标签：`production-path`、`real-backend`（仅在存在结构性成功条件时展示）

**还需确认**
- 运行模式：foreground，还是 background + `workspace_write` / `danger_full_access`？
- 跑到全部消除，还是限制在 N 次迭代？
- 除了 tsc 还有其他安全检查吗？
...
6. If run mode is still undecided, list it under "Need to confirm" and then ask the user to choose foreground or background. If background is on the table, also surface the execution policy explicitly. Do not omit the summary just because run mode is the only remaining blocker.
...
1. Require an explicit run-mode choice: **foreground** or **background**. For background launches, also surface the execution policy explicitly and default to `workspace_write` unless the user asks for unrestricted execution.
2. Confirm that the initialization/adoption or first-recovery git policy has already been captured through `git-runtime-governor` before the loop begins.
...
- This skill defaults that policy to `workspace_write` so detached runtime sessions stay sandboxed unless the user explicitly asks for unrestricted execution.
- Only switch to `danger_full_access` when the user explicitly asks for it or when reproducing sandbox-related blockers is itself the task.
