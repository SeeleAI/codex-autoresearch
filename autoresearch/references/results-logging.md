...
Define `<skill-root>` as the directory that contains the loaded execution `SKILL.md`. In the common repo-local install this is usually `.agents/skills/codex-autoresearch/autoresearch`, so the exact command becomes `python3 .agents/skills/codex-autoresearch/autoresearch/scripts/...`.
