...
Convert a goal into a validated, decision-complete `$codex-autoresearch` project and run plan.
...
Use this mode when the user knows what they want but not how to define the loop, or when the repo has not yet been initialized with the unified project system.

Under the root wrapper, uninitialized-repo plan flows should be preceded by an `env-bootstrap` readiness probe so the initialization plan does not assume missing hardware or tools.
...
For uninitialized repos:

- `plan` is mandatory.
- The wizard must collect all unresolved human-owned decisions in one continuous interrogation flow.
- The output is not just a launch block. It is a decision-complete implementation plan that explicitly commits later autonomous execution to the project document system plus the autoresearch runtime.
