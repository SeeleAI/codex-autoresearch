...
| Execution policy | no | prompt or env `AUTORESEARCH_EXECUTION_POLICY` (`workspace_write` by default; use `danger_full_access` only when you explicitly need unrestricted nested sessions) |
...
Before using `codex exec` in CI, configure Codex CLI authentication outside the skill itself. In controlled automation environments, this skill defaults to sandboxed `workspace_write`, so prefer `codex exec --full-auto ...` unless you explicitly need unrestricted `danger_full_access`. For programmatic runs, API key authentication is the preferred option.
...
| Execution policy | chosen during launch | `workspace_write` by default, `danger_full_access` only when explicitly requested |
...
Here `<skill-root>` is the directory containing the loaded execution `SKILL.md`. In the common repo-local install this is usually `.agents/skills/codex-autoresearch/autoresearch`.
...
- **health-check-protocol.md:** The standalone helper remains available, but exec mode does not automatically invoke the detached-runtime health preflight. CI wrappers may call `autoresearch_health_check.py` explicitly if they want the same structured integrity report before running.
