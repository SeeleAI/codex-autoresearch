...
All user-facing and helper JSON progress outputs must derive from the same `.agent-os/progress-snapshots.json` truth source.

Rules:

- `Progress Snapshot` is the single human-readable section name for numeric progress projection.
- Human-readable projection lines may use short labels, but must not add explanatory prose.
- Helper JSON outputs may expose:
  - `progress_snapshot`
  - `progress_snapshot_status`
- `progress_snapshot_status` must distinguish:
  - `persisted`
  - `calculated`
  - `uninitialized_project_system`
  - `error`
- Read-only status inspection must calculate a snapshot without appending to snapshot history.
- Snapshot history retention is governed by the snapshot file policy, not by caller-specific ad hoc logic.
...
If an exec-oriented helper emits progress metadata, it must keep the payload machine-readable and use the same `progress_snapshot` / `progress_snapshot_status` field names.
