...
Before any resume decision, verify that the unified project system exists. Missing root `AGENTS.md` or missing `.agent-os/project-index.md` means the repo is not initialized under the new model and must return to a forced plan/adoption flow before resume.

Under the root wrapper, run a lightweight `env-bootstrap` readiness probe before resuming so recovery does not continue through a broken hardware or toolchain context.
